...
export default function AdminPanel() {
      ...
      <div style={S.root}>
        <div style={S.hdr}>
          ...
          <a
            ...
            style={{ color: '#778', fontSize: 12, textDecoration: 'none' }}
          >
            ← Volver al sitio
          </a>
        </div>
  ...
  const filterDefs = [
    { key: 'all', label: `TODAS (${reservas.length})`, color: G },
    {
      key: 'pending',
      label: `PENDIENTES (${
        reservas.filter((r) => (r.status || 'pending') === 'pending').length
      })`,
      color: YELLOW,
    },
    {
    ...
    <div style={S.root}>
      ...
      <div style={S.countersGrid}>
        ...
        <div style={S.countCard}>
          ...
          <div style={S.countRow}>
            ...
            <span style={S.countSub}>
              aprobadas · $
              {reservas.reduce((sum, r) => sum + (r.combo?.price || 0), 0)}{' '}
              totales
            </span>
          </div>
      ...
      <div style={S.listWrap}>
        {filteredReservas.length === 0 && (
          <div style={S.empty}>
            <div style={{ fontSize: 30, marginBottom: 10, opacity: 0.4 }}>
              📋
            </div>
            <div>
              No hay reservas{' '}
              {filter !== 'all' ? `con estado "${filter}"` : 'todavía'}.
            </div>
          </div>
        )}
            ...
            <div key={r.id} style={S.resCard}>
              ...
              <div style={S.resGrid}>
                ...
                <div style={S.resField}>
                  ...
                  <div style={S.fv}>
                    {r.mesa ? (
                      `${r.mesa.type === 'v' ? 'VIP ' : 'Gen. '}#${r.mesa.n}`
                    ) : (
                ...
                <div style={S.resField}>
                  ...
                  <div style={S.fv}>
                    {r.combo ? (
                      `#${r.combo.id} ${r.combo.name} · $${r.combo.price}`
                    ) : (
                      <span style={{ color: '#556' }}>—</span>
                    )}
                  </div>
              ...
              <div style={S.resActions}>
                ...
                {isBusy && (
                  <span
                    style={{ fontSize: 11, color: '#556', alignSelf: 'center' }}
                  >
                    Procesando…
                  </span>
                )}
